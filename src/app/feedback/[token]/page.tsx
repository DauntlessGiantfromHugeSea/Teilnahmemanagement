    ...
    return (
      <main className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center px-4 py-10">
        <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-8 text-center">
          ...
          <h1 className="text-xl font-semibold mb-2">Vielen Dank für Ihr Feedback!</h1>
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-900 px-4 py-10">
      <div className="max-w-2xl mx-auto">
        <div className="mb-6 p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
        ...
        <form method="post" action={`/api/feedback/${invite.token}`} className="space-y-8">
          ...
          <div className="flex justify-end pt-4">
            <button className="inline-flex items-center gap-2 rounded-lg bg-blue-600 text-white px-5 py-2.5 text-sm font-semibold hover:bg-blue-700">
...
function renderInput(q: { id: string; type: string; options?: string[]; required?: boolean }) {
      ...
      <div className="space-y-2">
        {(q.options ?? []).map((o) => (
          <label key={o} className="flex items-start gap-2 text-sm cursor-pointer">
            <input type="checkbox" name={name} value={o} className="mt-1 h-4 w-4 accent-blue-600" />
            <span>{o}</span>
          </label>
        ))}
      </div>
  ...
  return (
    <div className="space-y-2">
      {(q.options ?? []).map((o) => (
        <label key={o} className="flex items-start gap-2 text-sm cursor-pointer">
          <input type="radio" name={name} value={o} required={q.required} className="mt-1 h-4 w-4 accent-blue-600" />
          <span>{o}</span>
        </label>
      ))}
    </div>
